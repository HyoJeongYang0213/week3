...
public:
	...
	TArray<CollisionInfo> debugContacts;
	...
	TArray<CollisionInfo> CheckCollisionAll(float t);

	// 무리(island) 단위로 재운다. 하나라도 타이머를 못 채우면 무리 전체가 깨어 있는다
	void UpdateSleep(float t, const TArray<pair<ACollider*, ACollider*>>& contacts);
...
private:
	...
	TMap<unsigned long long, CollisionInfo> previousManifolds;

...
void Renderer::CreateColorBuffer() { CustomColorBuffer = new ::ColorBuffer(); }
...
ID3D11ShaderResourceView *Renderer::LoadTexture(const std::wstring &filePath) {
  ...
  if (ext == L".dds") {
    hr = DirectX::LoadFromDDSFile(p.c_str(), DirectX::DDS_FLAGS_NONE, nullptr,
                                  image);
  } else {
    hr = DirectX::LoadFromWICFile(p.c_str(), DirectX::WIC_FLAGS_NONE, nullptr,
                                  image);
  }
...
void Renderer::CreateFrameBuffer() {
  ...
  ViewportInfo = {
      0,    0,   (FLOAT)framebufferdesc.Width, (FLOAT)framebufferdesc.Height,
      0.0f, 1.0f};
...
bool Renderer::CreateInputLayout(const D3D11_INPUT_ELEMENT_DESC *layoutDesc,
  ...
  HRESULT hr = Device->CreateInputLayout(layoutDesc, numElements,
...
ID3D11Buffer *Renderer::CreateVertexBuffer(const void *vertices,
                                           UINT byteWidth) {
...
void Renderer::CreateDepthStencil() {
  { // 깊이 버퍼용 텍스쳐 생성
    ...
    descDepth.Format =
        DXGI_FORMAT_D24_UNORM_S8_UINT; // Depth 24비트, Stencil 8비트
    ...
    if (SUCCEEDED(hr) && depthStencilBuffer) {
      Device->CreateDepthStencilView(depthStencilBuffer, nullptr,
                                     &depthStencilView);
  ...
  { // 기본
    ...
    dsDesc.FrontFace.StencilPassOp =
        D3D11_STENCIL_OP_REPLACE; // 통과 시 Ref 값으로 기록
    dsDesc.FrontFace.StencilFunc =
        D3D11_COMPARISON_ALWAYS; // 무조건 통과 (마스킹 단계)
  ...
  { // 기즈모용 깊이 스텐실
    //(깊이 테스트 비활성화로 항상 최상단 렌더링, 스텐실 마킹으로 외곽선 침범
    // 방지)
